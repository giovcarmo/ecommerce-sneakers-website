...
export function ProductLightbox({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 max-[450px]:hidden">
        ...
        <div className="flex gap-5 justify-center mt-5">
          {images.map((img, i) => (
            <img
              ...
              src={img.thumb}
              onClick={() => onSelect(i)}
              className={`hover:opacity-40 cursor-pointer w-20 rounded-md border-2 ${
                index === i ? "border-orange" : "border-transparent"
              }`}
            />
          ))}
        </div>
